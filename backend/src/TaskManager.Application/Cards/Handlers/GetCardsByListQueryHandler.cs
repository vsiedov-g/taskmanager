...
public class GetCardsByListQueryHandler : IRequestHandler<GetCardsByListQuery, IEnumerable<CardDto>>
{
    private readonly ICardRepository _cardRepository;

    public GetCardsByListQueryHandler(ICardRepository cardRepository)
    {
        _cardRepository = cardRepository;
    }

    public async Task<IEnumerable<CardDto>> Handle(GetCardsByListQuery request, CancellationToken cancellationToken)
    {
        var cards = await _cardRepository.GetByListIdAsync(request.ListId);
